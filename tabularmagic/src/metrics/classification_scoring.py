import numpy as np
import pandas as pd
from ...src.util.console import print_wrapped, color_text
...
class ClassificationMulticlassScorer:
    ...
    def _set_stats_df(self):
        ...
        if isinstance(y_pred, np.ndarray) and isinstance(y_true, np.ndarray):
            ...
            self._stats_df = cvdf.groupby(['Statistic'])[[self._name]].\
                mean().reindex(
                ['accuracy', 'f1', 'precision', 'recall', 'roc_auc(ovo)', 'n']
            )
    ...
    def _set_stats_by_class_df(self):
        ...
        if isinstance(self._y_pred, np.ndarray) and \
            isinstance(self._y_true, np.ndarray):
            try:
                roc_auc = roc_auc_score(self._y_true, self._y_pred_score, 
                        multi_class='ovr', average=None)
            except:
                roc_auc = np.nan
        elif isinstance(self._y_pred, list) and \
            isinstance(self._y_true, list):
            try:
                roc_auc = [roc_auc_score(y_true_elem, y_pred_score_elem, 
                            multi_class='ovr', average=None) 
                            for y_true_elem, y_pred_score_elem in 
                            zip(self._y_true, self._y_pred_score)]
            except:
                roc_auc = np.nan
        ...
        for i, pos_label in enumerate(self._y_pred_class_order):
            ...
            if isinstance(self._y_pred, np.ndarray) and isinstance(
                ...
                df.loc[len(df)] = pd.Series(
                    {
                        'Class': pos_label,
                        'Statistic': 'f1',
                        self._name: f1_score(self._y_true, self._y_pred, 
                                             labels=[pos_label],
                                             average='macro',
                                             zero_division=np.nan)
                    }
                ...
                df.loc[len(df)] = pd.Series(
                    {
                        'Class': pos_label,
                        'Statistic': 'precision',
                        self._name: precision_score(self._y_true, self._y_pred,
                                                    labels=[pos_label],
                                                    average='macro',
                                                    zero_division=np.nan)
                    }
                ...
                df.loc[len(df)] = pd.Series(
                    {
                        'Class': pos_label,
                        'Statistic': 'recall',
                        self._name: recall_score(self._y_true, self._y_pred, 
                                                 labels=[pos_label],
                                                 average='macro',
                                                 zero_division=np.nan)
                    }
                    ...
                    cvdf.loc[len(cvdf)] = pd.Series(
                        {
                            'Class': pos_label,
                            'Statistic': 'accuracy',
                            self._name: accuracy_score(y_true_elem, 
                                                       y_pred_elem),
                            'Fold': j
                        }
                    )
                ...
                self._stats_by_class_df = cvdf.groupby(['Class', 
                    'Statistic'])[[self._name]].mean()
