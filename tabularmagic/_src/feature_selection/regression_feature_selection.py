...
class LassoSelectorR(BaseFeatureSelectorR):
    ...
    def __init__(
        ...
        elif regularization_type is None:
            self.model = LinearRegression()
        else:
            raise ValueError(
                "Invalid input: regularization_type = " + f"{regularization_type}"
            )
    ...
    def select(self, dataemitter: DataEmitter, n_target_features: int):
        ...
        selector = SelectFromModel(
            estimator=self.model, prefit=True, max_features=n_target_features
        )
        selector.fit(X=X_train, y=y_train)
...
class RFESelectorR(BaseFeatureSelectorR):
    ...
    def select(self, dataemitter: DataEmitter, n_target_features: int):
        ...
        selector = RFE(estimator=self.model, n_features_to_select=n_target_features)
        selector.fit(X=X_train, y=y_train)
        self.selected_features = selector.get_feature_names_out()
        self.support = selector.get_support()
        return self.selected_features, self.support
