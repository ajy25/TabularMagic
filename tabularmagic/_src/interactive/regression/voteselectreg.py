...
from ...feature_selection.regression_feature_selection import BaseFeatureSelectorR
...
class RegressionVotingSelectionReport:
    ...
    def __init__(
        ...
        self._votes_df = pd.DataFrame.from_dict(
            self._selector_to_support, orient="index", columns=X_vars
        )
        ...
        self._top_features = self._vote_counts_series.sort_values(
            ascending=False
        ).index.to_list()[:n_target_features]
