...
def test_ols_scaling_simple(setup_data):
    ...
    # quick minmax test
    rmse_unscaled = analyzer.lm(
        target='y'
    ).metrics().loc['rmse', 'Ordinary Least Squares']
    ...
    rmse_scaled = report.metrics().loc['rmse', 'Ordinary Least Squares']
    ...
    rmse_unscaled_2 = report.metrics().loc['rmse', 'Ordinary Least Squares']
...
def test_ols_scaling_with_formula(setup_data):
    ...
    assert isinstance(report.model()._dataemitter.y_scaler(), LogTransformSingleVar)
    rmse_1 = report.metrics().loc['rmse', 'Ordinary Least Squares']
    ...
    assert isinstance(report.model()._dataemitter.y_scaler(), LogTransformSingleVar)
    rmse_2 = report.metrics().loc['rmse', 'Ordinary Least Squares']
...
def test_ml_scaling_simple(setup_data):
    ...
    report = analyzer.regress(
        models=[
            tm.ml.LinearR(name='ols')
        ],
        target='y'
    )
    rmse_unscaled = report.metrics().loc['rmse', 'ols']
    ...
    rmse_scaled = report.metrics().loc['rmse', 'ols']
    ...
    # once we reset the data, we should remove the y scaler
    analyzer.load_data_checkpoint()
    rmse_unscaled_2 = analyzer.regress(
        models=[
            tm.ml.LinearR(name='ols')
        ],
        target='y'
    ).metrics().loc['rmse', 'ols']
    ...
    assert isinstance(
        report.model('ols')._dataemitter.y_scaler(), 
        LogTransformSingleVar
    )
    ...
    report = analyzer.regress(
        models=[
            tm.ml.LinearR(name='ols'),
            tm.ml.TreeR(name='tree', n_trials=3)
        ],
        target='y'
    )
    assert isinstance(
    ...
    assert isinstance(
        report.model('tree')._dataemitter.y_scaler(), 
        Log1PTransformSingleVar
    )
