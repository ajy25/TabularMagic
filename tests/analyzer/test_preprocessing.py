...
def test_data_checkpoint_loading(setup_data):
    ...
    orig_emitted_X_numpy = analyzer.datahandler().train_test_emitter(
        y_var = 'SalePrice',
        X_vars = X_vars
    ).emit_train_test_Xy()[0].to_numpy()

    orig_emitted_y_numpy = analyzer.datahandler().train_test_emitter(
        y_var='SalePrice',
        X_vars=X_vars
    ).emit_train_test_Xy()[1].to_numpy()
    ...
    analyzer.select_vars(
        include_vars=categorical_vars_with_missing + numerical_vars
    )
    ...
    cat_missing_dropped_dataemitter_loaded_X_numpy = \
        analyzer.datahandler().train_test_emitter(
            y_var = 'SalePrice',
            X_vars = X_vars
        ).emit_train_test_Xy()[0].to_numpy()
